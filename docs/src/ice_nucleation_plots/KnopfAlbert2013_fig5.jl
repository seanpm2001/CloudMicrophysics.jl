...
Δa_w_ours = Vector{Float64}(undef, length(temp))
...
ours_J_ABIFM = Vector{Float64}(undef, length(temp))

it = 1
for T in temp
    Δa_w_ours[it] = CMO.Delta_a_w(prs, x_sulph_const, T)
    ours_J_ABIFM[it] = CMI.ABIFM_J(prs, dust_type, Δa_w_ours[it]) / 100^2

    #a_ice = exp((210368 + 131.438*T - (3.32373e6 /T) - 41729.1*log(T))/(8.31441*T)) # exp(large negative) = 0.0 right now
    a_ice = TD.saturation_vapor_pressure(thermo_params, T, TD.Ice()) / CMO.H2SO4_soln_saturation_vapor_pressure(prs, FT(0),T)
    a_sol = CMO.H2SO4_soln_saturation_vapor_pressure(prs, x_sulph[it],T) / CMO.H2SO4_soln_saturation_vapor_pressure(prs, FT(0),T)
    Δa_w[it] = max(a_sol - a_ice, FT(0.))
    J_ABIFM[it] = CMI.ABIFM_J(prs, dust_type, Δa_w[it]) / 100^2

    # plotting w constant x_sulph for comparison
    constx_a_sol = CMO.H2SO4_soln_saturation_vapor_pressure(prs, x_sulph_const,T) / CMO.H2SO4_soln_saturation_vapor_pressure(prs, FT(0),T)
    constx_Δa_w[it] = max(constx_a_sol - a_ice, FT(0.))
    constx_J_ABIFM[it] = CMI.ABIFM_J(prs, dust_type, constx_Δa_w[it]) / 100^2

    #println(Δa_w[it], " ", Δa_w_ours[it], " ", constx_Δa_w[it])
...
ax1 = MK.Axis(fig[1, 1], ylabel = "Δaw", xlabel = "Temp [K]")
MK.lines!(ax1, Δa_w_ours, temp, label = "Δa_w_ours")
MK.lines!(ax1, Δa_w, temp, label = "Δa_w_hack")
MK.lines!(ax1, constx_Δa_w, temp, label = "constx_Δa_w")
MK.axislegend()
fig
#
...
#MK.lines!(ax1, temp, ours_J_ABIFM, label = "ours x_sulph")
#MK.axislegend()
